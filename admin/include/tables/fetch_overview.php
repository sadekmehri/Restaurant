...
$columns = array('id_reservation', 'date_vente','id_table', 'nom', 'price','order_status');

$query = "SELECT * FROM `final` join product using(id_product) WHERE ";

if(verif($_POST["is_date_search"]) == "yes")
{
 $query .= 'date_vente BETWEEN "'.verif($_POST["start_date"]).'" 
            AND "'.verif($_POST["end_date"]).'" AND ';
}
...
while($data = mysqli_fetch_assoc($result))
{
	...
	{   
		$sub_array=array();
		$sub_array[] = $tab[$i]["id_reservation"];
		$sub_array[] = $tab[$i]["date_vente"];				    	
		$sub_array[] = $tab[$i]["table"];
		...
		{				    	  
				...
				$sub_array[] = '<button class="btn btn-dark" id="'.$tab[$i]["id_reservation"].'" "><span class="spinner-grow spinner-grow-sm"></span> Print</button>';
				$data[] = $sub_array;	
				$i++;						
    }
